...
option_mapping = st.sidebar.radio('Mappings', 
                                ['Lens','Snomed CT', 'MedCAT'])
...
if EXAMPLE_TEXT and not SCT_TEXT and not MCT_TEXT:
    ...
    extractor = lens_extractor(EXAMPLE_TEXT)
    all_entities = extractor.entities
    ...
    if opt_all:
        ...
        entities_lens = get_entities(EXAMPLE_TEXT)
        ...
        st.markdown('##### ⚙️ LENS Entities')
        st.markdown(extractor.visualize(get_ents(select_tags)), unsafe_allow_html=True)
...
# Assuming functions.lens2snomedct and functions.lens2medcat return a dictionary of entities
    ...
    entities_sct = lens2snomedct(SCT_TEXT)
    # Convert entities to a DataFrame for display
    ...
    entities_mct = lens2medcat(MCT_TEXT)
    # Convert entities to a DataFrame for display
